...
.layout{display:grid;grid-template-columns:minmax(0,1.45fr) minmax(330px,.55fr);gap:18px;padding:18px;max-width:1500px;margin:0 auto}
...
.video-wrap{background:#ede8de;border-radius:10px;display:grid;place-items:center;min-height:420px;overflow:hidden}video{max-width:100%;max-height:72vh;display:block;background:white}
.controls{display:grid;grid-template-columns:auto minmax(140px,1fr) auto auto auto;gap:10px;align-items:center;margin-top:12px}.btn,select{border:1px solid var(--line);background:#fffdf9;border-radius:8px;padding:8px 11px;font:inherit;cursor:pointer}.btn:hover{background:#f5f0e7}.btn.primary{background:#222;color:white;border-color:#222}.btn:disabled{opacity:.5;cursor:wait}
...
@media(max-width:900px){.layout{grid-template-columns:1fr}.video-wrap{min-height:280px}.controls{grid-template-columns:auto 1fr auto}.speed,.loop{grid-row:2}.side{min-height:0}}
</style>
</head>
<body>
...
<section class="card viewer">
  <div class="video-wrap"><video id="video" preload="metadata" playsinline src="/outputs/replay.mp4"></video></div>
  <div class="controls">
...
playBtn.onclick=()=>video.paused?video.play():video.pause();video.addEventListener('timeupdate',syncPlayer);video.addEventListener('loadedmetadata',syncPlayer);video.addEventListener('play',syncPlayer);video.addEventListener('pause',syncPlayer);seek.oninput=()=>{if(video.duration)video.currentTime=Number(seek.value)/1000*video.duration};$('#speed').onchange=e=>video.playbackRate=Number(e.target.value);$('#loop').onchange=e=>video.loop=e.target.checked;
...
async function loadProject(){project=await api('/api/project');order=[...project.current_order];renderOrder();renderHistory(project.history||[]);statusEl.textContent=`${project.units.length} 个主体 · ${project.event_count||0} events`;}
function reloadVideo(){const t=video.currentTime||0;video.src='/outputs/replay.mp4?v='+Date.now();video.load();video.addEventListener('loadedmetadata',()=>{video.currentTime=Math.min(t,video.duration||0)}, {once:true})}
